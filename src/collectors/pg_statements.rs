...
impl PGStatementsCollector {
    fn new(dbi: Arc<instance::PostgresDB>) -> anyhow::Result<Self> {
        ...
        let wal_bytes = IntGaugeVec::new(
            ...
            .subsystem("statements")
            .const_labels(dbi.labels.clone()),
            &["user", "database", "queryid", "wal"],
        )?;
        descs.extend(wal_bytes.desc().into_iter().cloned());
    ...
    fn collect(&self) -> Vec<proto::MetricFamily> {
        ...
        for row in data_lock.iter() {
            ...
            if wal_records > 0 {
                ...
                // WAL bytes by type
                self.wal_bytes
                    .with_label_values(&[user, database, query_id.as_str(), "fpi"])
                    .set(wal_fpi * block_size);
                self.wal_bytes
                    .with_label_values(&[user, database, query_id.as_str(), "regular"])
                    .set(wal_bytes);
        ...
        mfs.extend(self.wal_bytes.collect());
